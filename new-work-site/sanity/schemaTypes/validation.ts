...
export function documentIsVisible(context: {document?: unknown}): boolean {
  return isRecord(context.document) && context.document.visible === true
}
...
export function projectPublicationError(value: unknown): true | string {
  ...
  const safetyFlags = collectBlockingSafetyFlags(value)
  ...
  blocks.forEach((block, index) => {
    ...
    if (['heroImage', 'fullBleedImage', 'containedImage'].includes(String(block._type))) {
      if (!hasAssetReference(block.image)) problems.push(`${label} needs an image`)
      if (!isNonEmptyString(block.alt) && block.decorative !== true) {
    ...
    if (block._type === 'imagePair') {
      ;['left', 'right'].forEach((side) => {
        ...
        if (!item || !hasAssetReference(item.image)) problems.push(`${label} needs its ${side} image`)
        if (item && !isNonEmptyString(item.alt) && item.decorative !== true) {
    ...
    if (block._type === 'imageGrid') {
      ...
      images.forEach((item, imageIndex) => {
        if (!isRecord(item) || !hasAssetReference(item.image)) {
          problems.push(`${label} image ${imageIndex + 1} is missing`)
          return
        }
        if (!isNonEmptyString(item.alt) && item.decorative !== true) {
    ...
    if (['heroVideo', 'video', 'shortLoop'].includes(String(block._type))) {
      if (!hasAssetReference(block.poster)) problems.push(`${label} needs a poster`)
      ...
      if (!hasHostedSource && !hasRemoteSource) problems.push(`${label} needs a hosted or approved remote source`)
