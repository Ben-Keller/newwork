...
async function wideFeaturePlacementWarning(value: string | undefined, context: ValidationContext): Promise<true | string> {
  ...
  const precedingPortraits = await context.getClient({apiVersion: API_VERSION}).fetch(
...
export const coverMedia = defineType({
  ...
  fields: [
    ...
    defineField({name: 'focalPoint', title: 'Fixture focal point', type: 'focalPoint', group: 'crop'}),
    defineField({
      ...
      validation: (Rule) =>
        Rule.custom((value) =>
          value === true ? 'Replace this still-derived interaction test before public production use.' : true,
        ).warning(),
    }),
